/*
F * Copyright 2018, Cypress Semiconductor Corporation or a subsidiary of Cypress Semiconductor
...
* Application is notified by library whenever HRC configures heart rate notfications.
* Application stores bonded HRC address and notification configuration in NVRAM. This information is
* used in reconnections, after succesfull encryption, to start heart rate notiifcations automatically.
* Aplication sends heart rate notifications to HRC on every 1 minute, until HRC stops.
...
* 4. After connection with HRC, based on HRC reques, start, stop Heart Rate notifications and 
...
#if ( defined(CYW20735B0) || defined(CYW20735B1) || defined(CYW20719B1) || defined(CYW20719B0) || defined(CYW20721B1) )
...
#define HRS_PAIRED_KEYS_VS_ID   (WICED_NVRAM_VSID_START + 1)
...
static void                   hrs_event_cback(uint16_t conn_id, wiced_bt_hrs_event_type_t event_type);
...
#endif
{
    ...
    wiced_bt_hrs_initialize (hrs_event_cback);
...
wiced_result_t hrs_management_callback(wiced_bt_management_evt_t event, wiced_bt_management_evt_data_t *p_event_data)
{
    ...
    case BTM_BLE_ADVERT_STATE_CHANGED_EVT:
        ...
        {
            WICED_BT_TRACE("HRS ADV stooped \n");
        }
        break;
...
void hrs_set_advertisement_data(void)
{
    ...
    adv_elem[num_elem].len          = strlen(hrs_local_name);
...
wiced_bt_gatt_status_t hrs_gatts_req_read_handler(uint16_t conn_id, wiced_bt_gatt_read_t * p_data)
{
    ...
    if ( (p_data->handle >= HDLS_HEART_RATE) && ( p_data->handle <= HDLC_HEART_RATE_CONTROL_POINT ) )
    {
        return wiced_bt_hrs_process_client_read_req (conn_id, p_data);
    }

    // Check for a matching handle entry
    for (i = 0; i < hrs_gatt_db_ext_attr_tbl_size; i++)
    {
        if (hrs_gatt_db_ext_attr_tbl[i].handle == p_data->handle)
        {
            break;
        }
    }
    if (i == hrs_gatt_db_ext_attr_tbl_size)
    {
        WICED_BT_TRACE("read_hndlr attr not found hdl:%x\n", p_data->handle);
        return WICED_BT_GATT_INVALID_HANDLE;
    }

    attr_len_to_copy = hrs_gatt_db_ext_attr_tbl[i].cur_len;

    WICED_BT_TRACE("read_hndlr conn_id:%d hdl:%x offset:%d len:%d\n", conn_id, p_data->handle, p_data->offset, attr_len_to_copy);

    if (p_data->offset >= hrs_gatt_db_ext_attr_tbl[i].cur_len)
    {
        attr_len_to_copy = 0;
    }
    ...
    if (attr_len_to_copy != 0)
    {
        ...
        from = hrs_gatt_db_ext_attr_tbl[i].p_data + p_data->offset;
...
wiced_bt_gatt_status_t hrs_gatts_req_write_handler(uint16_t conn_id, wiced_bt_gatt_write_t * p_data)
{
    ...
    if ( (p_data->handle > HDLS_HEART_RATE) && ( p_data->handle <= HDLC_HEART_RATE_CONTROL_POINT_VALUE ) )
    {
...
void heart_rate_notify_timeout(uint32_t arg)
{
    ...
    wiced_bt_heart_rate_data_t  heart_rate_notification;
    memset(&heart_rate_notification, 0, sizeof(heart_rate_notification));
    
    heart_rate_notification.conn_id     = hrs_app_cb.conn_id;
...
#define HEART_BEAT_PER_MINUTE                   72 //typical value
    ...
    status = wiced_bt_hrs_send_heart_rate( &heart_rate_notification );
    WICED_BT_TRACE("notify heart rate %d \n", HEART_BEAT_PER_MINUTE);
}

void hrs_event_cback(uint16_t conn_id, wiced_bt_hrs_event_type_t event_type)
{
    wiced_result_t  result;
    uint8_t         nvram_write = 0;

    WICED_BT_TRACE("[%s] event_type:%d \n",__FUNCTION__, event_type);
    ...
    case WICED_BT_HRS_EVENT_HEART_RATE_NOTIFICATIONS_ENABLED:
        {
            ...
            wiced_start_timer(&hrs_app_cb.heart_rate_notify_timer, 10);
